  ...
  if (!diff.previous_run) {
    return (
      <p className="text-xs text-gray-500">
        First run for this job — {diff.added.length} item{diff.added.length === 1 ? '' : 's'} as the
        baseline.
      </p>
    );
  }
  ...
  return (
    <div className="space-y-3 text-xs">
      <p className="font-mono text-gray-500">
        vs run {diff.previous_run.id.slice(0, 8)} ·{' '}
        {new Date(diff.previous_run.started_at).toLocaleString()}
      </p>
      {nothing && <p className="text-gray-500">No changes since the previous run.</p>}
      {diff.added.length > 0 && (
        <Group tone="added" label={`Added (${diff.added.length})`} items={diff.added} />
      )}
